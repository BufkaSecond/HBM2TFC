...
public class TileEntityHopper extends TileEntity implements IHopper {
        ...
        if (iinventory != null) {
            ...
            if (iinventory instanceof ISidedInventory && b0 > -1) {
                ...
                for(int k = 0; k < aint.length; ++k) {
                    int injectorAllocatedLocal8 = aint[k];
                    if (redirect$zea000$hodgepodge$moveFromHopperToInventory(p_145891_0_, iinventory, injectorAllocatedLocal8, b0)) {
                        return true;
                    }
                }
            } else {
                ...
                for(int j = 0; j < i; ++j) {
                    if (redirect$zea000$hodgepodge$moveFromHopperToInventory(p_145891_0_, iinventory, j, b0)) {
                        return true;
                    }
                }
            }
    ...
    private static ItemStack func_145899_c(IInventory p_145899_0_, ItemStack p_145899_1_, int p_145899_2_, int p_145899_3_) {
        ...
        if (func_145885_a(p_145899_0_, p_145899_1_, p_145899_2_, p_145899_3_)) {
            ...
            int max;
            if (itemstack1 == null) {
                max = Math.min(p_145899_1_.getMaxStackSize(), p_145899_0_.getInventoryStackLimit());
    ...
    @MixinMerged(
        mixin = "com.mitchej123.hodgepodge.mixins.early.minecraft.MixinTileEntityHopper",
        priority = 1000,
        sessionId = "98d213e4-85ca-4ce4-959f-84858add440a"
    )
    private static boolean redirect$zea000$hodgepodge$moveFromHopperToInventory(IHopper hopper, IInventory inventory, int slot, int side) {
        ItemStack contentSlot = inventory.getStackInSlot(slot);
        if (contentSlot != null && contentSlot.stackSize != 0) {
            if (inventory instanceof ISidedInventory) {
                ISidedInventory sidedInventory = (ISidedInventory)inventory;
                if (!sidedInventory.canExtractItem(slot, contentSlot, side)) {
                    return false;
                }
            }
    ...
    private static int getSpaceSlot(IHopper hopper, ItemStack is) {
        ...
        for(int i = 0; i < size; ++i) {
            ...
            if (space.stackSize < Math.min(space.getMaxStackSize(), hopper.getInventoryStackLimit()) && is.isItemEqual(space) && ItemStack.areItemStackTagsEqual(is, space)) {
                return i;
            }
        }
