...
public abstract class EntityLivingBase extends Entity {
    ...
    public void removePotionEffect(int p_82170_1_) {
        PotionEffect potioneffect = (PotionEffect)this.activePotionsMap.remove(p_82170_1_);
        if (potioneffect != null) {
            this.onFinishedPotionEffect(potioneffect);
        }
        ...
        if (!ForgeHooks.onLivingDeath(this, p_70645_1_)) {
            ...
            if (!this.worldObj.isRemote) {
                ...
                if (!ForgeHooks.onLivingDrops(this, p_70645_1_, this.capturedDrops, i, this.recentlyHit > 0, j)) {
                    Iterator var6 = this.capturedDrops.iterator();

                    while(var6.hasNext()) {
                        EntityItem item = (EntityItem)var6.next();
    ...
    private int getArmSwingAnimationEnd() {
        return this.modifyReturnValue$zbk000$hodgepodge$fixHasteArmSwing(this.isPotionActive(Potion.digSpeed) ? 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1 : (this.isPotionActive(Potion.digSlowdown) ? 6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2 : 6));
    }
    ...
    public void moveEntityWithHeading(float p_70612_1_, float p_70612_2_) {
        ...
        f4 = MathHelper.sqrt_double(d0 * d0 + d1 * d1) * 4.0F;
        if (f4 > 1.0F) {
            f4 = 1.0F;
        }
            ...
            if (!this.worldObj.isRemote) {
                ...
                for(int j = 0; j < 5; ++j) {
                    ...
                    if (!ItemStack.areItemStacksEqual(itemstack1, itemstack)) {
                        EntityTracker var10000 = ((WorldServer)this.worldObj).getEntityTracker();
                        this.handler$zjg000$gtnhlib$addEquipmentChangeEventBus((CallbackInfo)null, j, itemstack, itemstack1);
                        var10000.func_151247_a(this, new S04PacketEntityEquipment(this.getEntityId(), j, itemstack1));
                        if (itemstack != null) {
                            this.attributeMap.removeAttributeModifiers(itemstack.getAttributeModifiers());
                        }
    ...
    @MixinMerged(
        mixin = "com.mitchej123.hodgepodge.mixins.early.minecraft.MixinEntityLivingBase_FixHasteArmSwing",
        priority = 1000,
        sessionId = "98d213e4-85ca-4ce4-959f-84858add440a"
    )
    private int modifyReturnValue$zbk000$hodgepodge$fixHasteArmSwing(int original) {
        return Math.max(original, 2);
    }

    @Overwrite
    @MixinMerged(
        mixin = "com.mitchej123.hodgepodge.mixins.early.minecraft.MixinEntityLivingBase_FixPotionException",
        priority = 1000,
        sessionId = "98d213e4-85ca-4ce4-959f-84858add440a"
    )
    protected void updatePotionEffects() {
        ...
        try {
            while(iterator.hasNext()) {
                PotionEffect potioneffect = (PotionEffect)this.activePotionsMap.get(iterator.next());
        ...
        } catch (ConcurrentModificationException var25) {
        }
        ...
        if (i > 0) {
            ...
            if (spawnParticle) {
                ...
                String var10001 = ambient ? "mobSpellAmbient" : "mobSpell";
                double var10002 = this.posX + (this.rand.nextDouble() - 0.5D) * (double)this.width;
                double var10003 = this.posY + this.rand.nextDouble() * (double)this.height - (double)this.yOffset;
                double injectorAllocatedLocal17 = this.posZ + (this.rand.nextDouble() - 0.5D) * (double)this.width;
                double injectorAllocatedLocal15 = var10003;
                double injectorAllocatedLocal13 = var10002;
                String injectorAllocatedLocal12 = var10001;
                World injectorAllocatedLocal11 = var10000;
                if (this.wrapWithCondition$zeo000$hodgepodge$showParticles(injectorAllocatedLocal11, injectorAllocatedLocal12, injectorAllocatedLocal13, injectorAllocatedLocal15, injectorAllocatedLocal17, r, g, b)) {
                    injectorAllocatedLocal11.spawnParticle(injectorAllocatedLocal12, injectorAllocatedLocal13, injectorAllocatedLocal15, injectorAllocatedLocal17, r, g, b);
                }
            }
        }
    ...
    @MixinMerged(
        mixin = "com.gtnewhorizon.gtnhlib.mixins.early.MixinEntityLivingBase",
        priority = 1000,
        sessionId = "98d213e4-85ca-4ce4-959f-84858add440a"
    )
    private void handler$zjg000$gtnhlib$addEquipmentChangeEventBus(CallbackInfo ci, int slot, ItemStack prevItem, ItemStack newItem) {
        MinecraftForge.EVENT_BUS.post(new LivingEquipmentChangeEvent(this, slot, prevItem, newItem));
    }

    @MixinMerged(
        mixin = "com.mitchej123.hodgepodge.mixins.early.minecraft.MixinEntityLivingBase_HidePotionParticles",
        priority = 1001,
        sessionId = "98d213e4-85ca-4ce4-959f-84858add440a"
    )
    private boolean wrapWithCondition$zeo000$hodgepodge$showParticles(World instance, String particleName, double x, double y, double z, double velocityX, double velocityY, double velocityZ) {
        return !TweaksConfig.hidePotionParticlesFromSelf || this != Minecraft.getMinecraft().thePlayer;
    }
}
