...
public class FontRenderer implements IResourceManagerReloadListener {
    ...
    @Overwrite
    @MixinMerged(
        mixin = "com.mitchej123.hodgepodge.mixins.early.minecraft.MixinFontRenderer",
        priority = 1000,
        sessionId = "98d213e4-85ca-4ce4-959f-84858add440a"
    )
    public String wrapFormattedStringToWidth(String str, int wrapWidth) {
        int firstLineWidth = Math.max(1, this.sizeStringToWidth(str, wrapWidth));
        if (str.length() <= firstLineWidth) {
            ...
            while(true) {
                int lineWidth = Math.max(1, this.sizeStringToWidth(str, wrapWidth));
                String line = StringUtils.substring(str, 0, lineWidth);
                output.append(line);
                ...
                char nextChar = str.charAt(lineWidth);
                boolean nextIsBlank = nextChar == ' ' || nextChar == '\n';
